...
restoredefaultpath;
addpath(('C:\Users\assad\OneDrive - Ulster University\Toolboxes\ironclust-master\matlab'));
...
analysed_folder = fullfile(folder_name, 'Raw');
...
% Save the data as a mda file
% recordingFile = fullfile(folder_name, 'recording_data.bin');
recordingFile = write_mda(data, analysed_folder, sampling_rate);

% Usage
prmFile = create_prm_file(recordingFile, sampling_rate);

% Define paths
outputDir = fullfile(folder_name, 'Sorted');
...
irc2 select 'single_unit.prm'

% Run IronClust sorting
irc2(recordingFile, outputDir);
irc2_traces(fullfile(outputDir, 'raw_geom.prm'));